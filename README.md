...
- 完整数据部署下，88 个子类和 102 个 Benchmark 全部可评测
...
页面右上角的“全子类快速检测”会用当前模型对 88 个可评测子类各运行 1 个真实样例。任务共享一个已拉起的模型服务，页面会显示总进度，并将结果汇总到 `result/current/`。该操作会替换当前结果，适合部署后验收 Benchmark 数据、评测适配器、进度回传和结果展示链路。
