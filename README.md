...
XiliLake 是 MiLab 建设的大语言模型与多模态模型可信评测系统。系统统一管理通用、医疗和网络安全领域的 Benchmark，支持总评测、领域、大类、子类和单个 Benchmark 五级评测，能够启动本地或 OpenAI 兼容接口模型、实时查看任务进度，并展示指标与完整样例结果。当前结果可导出完整 ZIP，总结果表可导出 CSV。
