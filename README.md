...
**多模态大模型可信测评系统-XiliLake v0.1**

XiliLake 是 MiLab 建设的大语言模型与多模态模型可信评测系统。系统统一管理通用、医疗和网络安全领域的 Benchmark，支持总测评、领域、大类、子类和单个 Benchmark 五级评测，能够启动本地或 OpenAI 兼容接口模型、实时查看任务进度，并展示指标与完整样例结果。当前结果可导出完整 ZIP，总结果表可导出 CSV。
...
- 100 子类
- 113 Benchmark
- 完整数据部署下，100 子类和 113 Benchmark 全部可评测
...
| 评测领域 | 评测大类 |
| --- | --- |
| 通用评测 | 基本事实准确性、感知内容真实性、推理决策可靠性、任务遵循可靠性、伪造识别准确性、隐私信息安全性、攻击抵御鲁棒性、社会群体公平性、法律法规遵守性、伦理道德符合性 |
| 医疗行业评测 | 医疗事实准确性（5 子类） |
| 网络安全行业评测 | 网安知识准确性、漏洞识别准确性 |
...
访客页面可在大类、子类或单个 Benchmark 三个层级直接启动评测。大类和子类综合评测会复用同一个已加载模型，依次运行当前范围内的全部 Benchmark，并将统一进度、汇总指标和结果样例写入 `result/current/`；再次评测同一模型时，不同 Benchmark 的缓存结果会独立保留或更新。
