...
3. 大类按“直接判定对象”划分：事实真伪、推理过程、任务完成、攻击或有害请求抵御、用户隐私保护、群体差异、法律规则或伦理价值。用户隐私安全性只保留个人信息、财产与机密访问任务；既有内容的安全判断进入伦理道德符合性；诱导模型提供有害帮助的请求进入攻击抵御鲁棒性。
...
三个安全相关大类按照输入与直接判定对象互斥划分：攻击抵御鲁棒性判断“风险输入能否突破边界”，用户隐私安全性判断“隐私、财产或机密能否得到保护”，伦理道德符合性判断“既有内容或行为是否符合安全伦理”。
...
| 用户隐私安全性 | 个人信息与财产侵害请求拒答评测 | 请求本身要求泄露个人信息、破解凭据或侵害财产，不依赖显式授权关系 | XSafety-Privacy-Refusal |
| 用户隐私安全性 | 信息采集与使用隐私合理性评测 | 结合信息类型、收集主体和使用目的判断信息采集与使用是否合理 | ConfAIde-Tier2 |
| 用户隐私安全性 | 未授权机密信息泄露防护评测 | 在给定所有者、请求者和访问规则后判断是否应披露机密；判定随权限变化 | RuLES |
...
XSafety 按输入机制拆分为互斥数据：17,990 条普通风险请求进入攻击抵御鲁棒性，2,000 条 Privacy And Property 进入用户隐私安全性，2,000 条 Goal Hijacking 用于越狱攻击防御。Prompt Leaking 和 Role Play Instruction 共 3,998 条因标签与实际内容混杂而不参与评分，三个可评测分片的样本 ID 交集为 0。
