...
> 核验日期：2026-08-18。核验对象为网页实际加载的 109 个 Benchmark 实例（96 个唯一名称）。逐子类、逐 Benchmark 的当前归属、介绍、分类核验结论和原文链接见 [current_taxonomy_full.md](current_taxonomy_full.md)。
...
- 当前网页实际计数为 3 个评测领域、11 个可见大类、86 个子类、109 个 Benchmark，全部标记为可评测。“医疗基本事实准确性”暂无符合定义的本地 Benchmark，因此不用 MedSafetyBench 填充该类。

## 法律法规遵守性边界

法律法规遵守性完整覆盖 LegalBench 的 162 个官方任务。系统保留原有三个独立入口，并把其余 159 个任务汇总为七个无任务重叠的可评测套件；共接入 90,885 条有 gold 的测试样本。`ssla_company_defendants` 中 9 条官方空标签记录不参与评分。

| 子类 | 唯一判定边界 | LegalBench 任务数 | 有标签测试样本数 |
| --- | --- | ---: | ---: |
| 隐私政策法规遵守性评测 | 隐私政策是否蕴含、披露或支持数据处理规则 | 11 | 24,516 |
| 消费者权益法规遵守性评测 | 消费合同权利、不公平条款及电话营销规则 | 3 | 4,256 |
| 法律争点识别准确性评测 | 从事实陈述识别需要处理的法律问题，不给最终结论 | 17 | 11,599 |
| 法律规则知识准确性评测 | 回忆或识别规则、司法行为规范及判例法源 | 5 | 9,809 |
| 法律规则适用结论准确性评测 | 在规则领域已知时把规则应用到事实并得出结论 | 11 | 2,180 |
| 合同与法律文本解释准确性评测 | 解释合同、成文法、政策及专业法律文本的含义或蕴含关系 | 105 | 32,286 |
| 司法文本结构与修辞分析评测 | 识别裁判文本结构、定义、论证功能、因果和先例关系 | 10 | 6,239 |

任务归属直接采用 LegalBench 官方 `ISSUE_TASKS`、`RULE_TASKS`、`CONCLUSION_TASKS`、`INTERPRETATION_TASKS` 和 `RHETORIC_TASKS` 五个互斥列表；隐私和消费者任务从官方列表中定向抽出，且三个原有任务从聚合套件中排除，因此 162 个任务恰好各归属一次。
